...
class PluginLocationService {
  static Directory get location =>
      Directory("C:\\Users\\awallen\\Development\\AppFlowy\\dynamic_plugin_demo\\lib\\plugins");
}
...
class FlowyPluginService {
  ...
  bool _isFlowyPluginDirectory(FileSystemEntity entity) => entity is Directory;
  ...
  Future<void> _initialize() async {
    for (final lib in await _libs) {
      ...
      final flowyPlugin = _runtime.executeLib('package:${lib.package}/plugin.dart', 'registerPlugin') as $Instance;
      _plugins.add(flowyPlugin.$value);
    }
  }

  final List<FlowyPlugin> _plugins = [];
  ...
  Iterable<FlowyPlugin> get plugins => List.from(_plugins);
}
...
class _PluginLib {
  ...
  factory _PluginLib.fromDirectory(Directory directory) {
    ...
    return _PluginLib(
      ...
      sources: directory.listSync().whereType<File>().toList(),
    );
  }
  ...
  Map<String, String> get sources => _sources.fold(
      {},
